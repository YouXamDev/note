...
周期信号可以表示为傅里叶级数形式 $$f_T(t)=\sum_{n=-\infty}^{\infty}F_ne^{jn\omega_0t}$$

其傅里叶变换为 $$F_T(\omega)=\sum_{n=-\infty}^{\infty} F_n \cdot 2\pi \delta(\omega-n\omega_0)=2\pi\sum_{n=-\infty}^{\infty} F_n \cdot \delta(\omega-n\omega_0)$$
