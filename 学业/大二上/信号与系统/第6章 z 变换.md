...
其中 $$B_j=\dfrac 1{(s-j)!}\left[\dfrac{\mathbf{d}^{s-j}}{\mathbf{d}z^{s-j}}(z-z_i)^s\dfrac{X(z)}z\right]$$
例如：$$\dfrac{X(z)}z=\dfrac 1{z(z-1)^2}=\dfrac{B_1}{z-1}+\dfrac{B_2}{(z-1)^2}+\dfrac{B_3}z$$
