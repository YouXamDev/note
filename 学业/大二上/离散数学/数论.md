...
$a \equiv b\ (\mod m) \Leftrightarrow a(\mod m) \equiv b(\mod m)$
$\Leftrightarrow \exists k \in \mathbf{Z},a=b+km$
...
如果 $p$ 是质数并且 $a$ 不是 $p$ 的倍数，则 $a^{p-1} \equiv 1(\mod p)$。

该定理可以用来求逆元，两边同时乘 $a^{-1}$ : $a^{p-2}\equiv a^{-1}(\mod p)$。即：$a$ 的逆元是 $a$ 的 $p-2$ 次方，其中 $p$ 是质数。
## 扩展欧几里得定理
...
2 是 11 的原根，因为在模 11 的意义下 $2^1 \equiv 2, 2^2 \equiv 4, 2^3 \equiv 8, 2^4 \equiv 5, 2^5 \equiv 10, 2^6 \equiv 9, 2^7 \equiv 7, 2^8 \equiv 3, 2^9 \equiv 6, 2^{10} \equiv 1$ 。

3 不是 11 的原根，因为在模 11 的意义下 $3^1 \equiv 3^6 \equiv 3$。

# 离散对数(Discrete Logarithms)

若 $p$ 是质数，且 $r^e \equiv a (\mod p)$ ， $0 \le e \le p-1$，则称 $e$ 是在模 $p$ 意义下以 $r$ 为底 $a$ 的对数，$\log_ra=e$。